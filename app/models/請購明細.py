...
class 請購明細(db.Model):
    ...
    狀態 = Column(String) # 等待物料更新 / 正在集單 / 請購簽核進行中 / 退件等待處置 / 採購中等待收貨 / 等待驗收 / 已結案
    狀態更新時間戳 = Column(DateTime(timezone=True))

    資材部請購單號 = Column(String, ForeignKey('請購彙總.單號'))
    ...
    # 建立到 { 捷拓廠區, 物料, 專案, 請購彙總, 收貨驗收紀錄, 員工 } 的關聯
    ...
    請購彙總表單 = relationship("請購彙總", back_populates="請購明細")
    ...
    # 多對多關係
    附件檔案 = relationship("檔案", secondary='請購明細附件檔案關聯表', back_populates='請購明細')
